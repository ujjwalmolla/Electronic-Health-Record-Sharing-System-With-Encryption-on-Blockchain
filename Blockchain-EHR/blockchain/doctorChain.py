...
class MainWindow(QDialog):
    ...
    def conv_dict(self,details):
        ...
        return res
    def loaddata(self):
        api_url = "http://127.0.0.1:{}/doctorchain".format(self.port)
        pr = {'doctor': self.doctor}
        ...
        for dt in dict_chain:
            for tx in dt['transactions']:
                print(type(tx["details"]),tx["details"])
                # d = self.conv_dict(tx["details"])
                d = tx["details"]
                # print(d)
                # print(d.keys())
                # print(d["medicine"] , d["test"])
                # print(len(d))
